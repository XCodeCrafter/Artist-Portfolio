...
import {
  ...
  type MusicPreviewSelectionMessage,
} from "@/components/music/MusicPageView";
import { MUSIC_PREVIEW_READY_MESSAGE } from "@/components/admin/v2/MusicPreviewRuntime";
import {
  MUSIC_PREVIEW_UPDATE_MESSAGE,
  type MusicEditorDraft,
  type MusicEditorFooter,
  type MusicEditorSection,
  type MusicPreviewUpdateMessage,
} from "@/lib/admin/music-editor";

export type MusicPreviewDevice = "desktop" | "mobile";

export const MUSIC_PREVIEW_VIEWPORTS = {
  desktop: { width: 1440, height: 900 },
  mobile: { width: 390, height: 844 },
} as const;

function isSelectionMessage(value: unknown): value is MusicPreviewSelectionMessage {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const candidate = value as Record<string, unknown>;
  return (
    candidate.type === MUSIC_PREVIEW_SELECTION_MESSAGE &&
    ["hero", "platforms", "spotify", "soundcloud"].includes(
      String(candidate.section)
    )
  );
}

export default function MusicPreviewFrame({
  device,
  draft,
  focusRequestId,
  footer,
  ...
  isLive: boolean;
  onSelectSection: (section: MusicEditorSection) => void;
  selectedSection: MusicEditorSection;
}) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const stageRef = useRef<HTMLDivElement | null>(null);
  const frameRef = useRef<HTMLIFrameElement | null>(null);
  const onSelectSectionRef = useRef(onSelectSection);
  const viewport = MUSIC_PREVIEW_VIEWPORTS[device];
  const message = useMemo<MusicPreviewUpdateMessage>(
    () => ({
      type: MUSIC_PREVIEW_UPDATE_MESSAGE,
      draft,
      footer,
      focusRequestId,
      selectedSection,
  ...
  useEffect(() => {
    function receivePreviewMessage(event: MessageEvent) {
      ...
      if (
        ...
        event.data.type === MUSIC_PREVIEW_READY_MESSAGE
      ) {
        sendDraft();
        return;
      }
    ...
    >
      {!isLive ? (
        <div
          ...
          role="status"
        >
          Layout example · sample content, not the live Music page
        </div>
      ) : null}
      <div className="admin-scrollbar-none flex min-h-[520px] justify-center overflow-auto rounded-[18px] bg-black/50 p-3">
        ...
        >
          <iframe
            aria-label={`Music page ${device} preview`}
            className="absolute left-0 top-0 block origin-top-left border-0 bg-black"
            onLoad={sendDraft}
            ref={frameRef}
            src="/admin/v2-preview/music"
            style={{ width: viewport.width, height: viewport.height }}
            title={`Music page preview at ${viewport.width} pixels`}
          />
        </div>
      </div>
    </div>
